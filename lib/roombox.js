 ...
 * - interval: interval in milliseconds between segments
 * @param {Function} cb: standard cb(err, result) callback
 */
Roombox.prototype.play = function (trackNumber, opts, cb) {
  ...
  function play(segment, cb) {
    ...
    setTimeout(cb, opts.interval || (INTERVAL + (segment.duration * 1000 / 64)));
  }
